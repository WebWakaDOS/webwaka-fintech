...
insuranceRouter.get('/policies', requireRole(['admin', 'agent', 'customer']), async (c) => {
  ...
  const query = user.role === 'customer' 
    ? 'SELECT * FROM fint_insurancePolicies WHERE tenantId = ? AND customerId = ? ORDER BY createdAt DESC'
    : 'SELECT * FROM fint_insurancePolicies WHERE tenantId = ? ORDER BY createdAt DESC';
...
insuranceRouter.post('/policies', requireRole(['admin', 'agent']), async (c) => {
  ...
  await c.env.DB.prepare(
    'INSERT INTO fint_insurancePolicies (id, tenantId, customerId, policyType, premiumKobo, coverageKobo, status, startDate, endDate, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
  )
